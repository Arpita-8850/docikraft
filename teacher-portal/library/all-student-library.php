...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
	...
	<body>
		<?php
			...
			error_reporting(0);
			$query = "SELECT * FROM student s, student_library_map sl, library l WHERE s.s_id= sl.s_id and sl.book_id=l.book_id  ORDER BY sl.s_id";
				...
				<main class="h-full pb-16 overflow-y-auto">
					...
					<!-- Pie chart START -->
						<?php
							...
							$sql = "SELECT COUNT(dues) as pending FROM student_library_map where dues>0";
							$result = mysqli_query($conn, $sql);
							$count = mysqli_fetch_assoc($result);						
							$pending = $count['pending'];

							$sql_query = "SELECT COUNT(dues) as done FROM student_library_map where dues=0";
										...
										while($result = mysqli_fetch_assoc($data))
										{
											echo"
												<tr class='text-gray-700 dark:text-gray-400'>
													...
													<td class='px-4 py-3 text-xs'>".$result['librarystatus']."</td>
													<td class='px-4 py-3 text-sm'>Rs. ".$result['dues']."</td>
													<td class='px-4 py-3 text-sm'>".$result['book_id']."</td>
													<td class='px-4 py-3 text-sm'>".$result['book_name']."</td>
													<td class='px-4 py-3'>
														<div class='flex items-center space-x-4 text-sm'>
														
															<button class='flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray' aria-label='Edit'>
																<a href='UpdateLibrary.php?rollno=$result[s_id]&name=$result[first_name]&book_id=$result[book_id]'>
