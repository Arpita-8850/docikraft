	...
	<body>
						...
						<form  action="" method = "POST"> 
							<?php
								...
								$records = mysqli_query($conn,"SELECT * FROM student s, 
																			 student_library_map sl, 
																			 library l 
																		WHERE sl.book_id=l.book_id 
																		AND sl.book_id = '$book_id'
																		AND s.s_id = sl.s_id 
								...
								{ 
									echo' 
										<div class="container px-6 mx-auto grid">
											...
											<p class="my-2 text-1xl text-gray-700 dark:text-gray-200"><b>Book Id:</b> '.$data['book_id'].' </p>
											<p class="my-3 text-1xl text-gray-700 dark:text-gray-200"><b>Book name:</b> '.$data['book_name'].' </p>
											<br>
		...
		<?php		
			if($_POST['submit'])
			{
				...
				$query = "UPDATE student_library_map SET librarystatus='$librarystatus', dues='$dues'  WHERE s_id='$rollno' AND book_id='$book_id'";
				$data = mysqli_query($conn, $query);
				if($data)
				{
					echo "<script type='text/javascript'>alert('Data updated.');  location.replace('all-student-library.php') </script>";
				}
				else
				{
					echo "<script type='text/javascript'>alert('Data not updated. Please try again later.');  location.replace('library-home.php') </script>";
				}
			}
		?>
	</body>
