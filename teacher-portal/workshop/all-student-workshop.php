...
<html :class="{ 'theme-dark': dark }" x-data="data()" lang="en">
	...
	<body>
		<?php
			...
			error_reporting(0);
			$query = "SELECT * FROM student s, student_workshop_map sw, workshop w WHERE s.s_id= sw.s_id AND sw.too_id=w.tool_id ORDER BY s.s_id";
		...
		<div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen }">		
			<!-- Desktop sidebar START -->
				...
				<div class="py-4 text-gray-500 dark:text-gray-400">
					<a class="ml-6 text-lg font-bold text-gray-800 dark:text-gray-200" href="workshop-home.php">
					CRCE Workshop
					</a>
					<ul class="mt-6">
					<li class="relative px-6 py-3">
						<a class="inline-flex items-center w-full text-sm font-semibold transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200"
						href="workshop-home.php"
						>
					...
					<li class="relative px-6 py-3">
						...
						>
							...
							<a class="w-full" href="all-student-workshop.php">All Student</a>
							</li>
							<li class="px-2 py-1 transition-colors duration-150 hover:text-gray-800 dark:hover:text-gray-200">
							<a class="w-full" href="workshop-dues.php"> Dues </a>
							</li>
						</ul>
						</template>
					</li>
					...
					<div class="px-6 my-6">
						<button class="flex items-center justify-between w-full px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-purple-600 border border-transparent rounded-lg active:bg-purple-600 hover:bg-purple-700 focus:outline-none focus:shadow-outline-purple">
							<a href="add-workshop-tool.php">
								Add New Tool <span class="ml-2" aria-hidden="true">+</span>
							</a>
						</button>
					</div>
				</div>
			...
			<div class="flex flex-col flex-1 w-full">
				...
				<main class="h-full pb-16 overflow-y-auto">
					...
					<h2 class="my-6 text-2xl font-semibold text-gray-700 dark:text-gray-200"> Students Data</h2>
					...
					<!-- Pie chart START -->
						<?php
							...
							$sql = "SELECT COUNT(dues) as pending FROM student_workshop_map where dues>0";
							$result = mysqli_query($conn, $sql);
							$count = mysqli_fetch_assoc($result);						
							$pending = $count['pending'];

							$sql_query = "SELECT COUNT(dues) as done FROM student_workshop_map where dues=0";
							$output = mysqli_query($conn, $sql_query);
							$done_count = mysqli_fetch_assoc($output);						
							$done = $done_count['done'];
						?>

						<div class="min-w-0 p-3 bg-white rounded-lg shadow-xs dark:bg-gray-800">
							<h4 class="mb-4 font-semibold text-gray-800 dark:text-gray-300">
								Students Dues Pie Chart
							</h4>
							<canvas id="myData" width="200" height="50"></canvas>
										...
										while($result = mysqli_fetch_assoc($data))
										{
											echo"
												<tr class='text-gray-700 dark:text-gray-400'>
													...
													<td class='px-4 py-3 text-xs'>".$result['workshopstatus']."</td>
													<td class='px-4 py-3 text-sm'>Rs. ".$result['dues']."</td>
													<td class='px-4 py-3 text-sm'>".$result['too_id']."</td>
													<td class='px-4 py-3 text-sm'>".$result['tool_name']."</td>
													<td class='px-4 py-3'>
														<div class='flex items-center space-x-4 text-sm'>
														
															<button class='flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray' aria-label='Edit'>
																<a href='UpdateWorkshop.php?rollno=$result[s_id]&name=$result[first_name]&tool_id=$result[too_id]'>
